...
const RecipeCard = ({ title, image, id }) => {
    return (
        <Link to={`/recipes/${id}`} className="group">
            <div className="w-44 aspect-square rounded-xl overflow-hidden shadow-lg border border-green-400 relative flex-shrink-0 transition-transform duration-200 group-hover:scale-105 group-hover:shadow-xl bg-white">
                <img
                    src={image}
                    alt={title}
                ...
                <div className="absolute top-2 right-2 bg-white/80 rounded-full p-1 shadow group-hover:bg-orange-100 transition-colors">
